# Changelog

Engineering Expert Curriculumの利用者に影響する変更を記録します。形式は[Keep a Changelog](https://keepachangelog.com/en/1.1.0/)を基礎とし、versionは[Semantic Versioning](https://semver.org/spec/v2.0.0.html)に従います。教材の訂正詳細は[ERRATA.md](ERRATA.md)にも残します。

## [Unreleased]

### Added

- 全情報を保持するno-JS baseline上で、承認済みsimulationを操作できるdependency-free first-party progressive runtime。

### Security

- exact v0.2 meta CSP、40 KiB JavaScript source policy、network/storage/analyticsを禁止するruntime境界。

## [0.1.0] - 2026-07-31

### Added

- 1,140項目を保持する、ブラウザ検索可能な静的カタログ。
- Learn → Practice → Explain → Prove → Transfer → Reviewを実行する30のコアレッスン。
- 6つの習熟ゲート、前提ロードマップ、CS2023・SWEBOK V4.0a・SFIA 9対応表。
- グローバルサービス、レガシー進化、OSS公開を扱う3つのCapstone。
- JavaScriptを使わず`file://`とGitHub Pagesで読めるHTML/CSS成果物。
- MIT License、日英README、貢献、行動規範、governance、security、Errata、citationの公開運営文書。

### Security

- authored HTML、structured content、path、決定的publication boundaryを検証するdependency-free build契約。
- 脆弱性をGitHubの非公開advisoryで受け付ける報告方針。

[Unreleased]: https://github.com/albert-einshutoin/engineering-expert-curriculum/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/albert-einshutoin/engineering-expert-curriculum/releases/tag/v0.1.0
